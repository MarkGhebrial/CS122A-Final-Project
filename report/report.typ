...
*Our current project description is as follows:* The FPGA acts as an SPI master for two devices: the SD card and the Raspberry Pi Pico. The FPGA initializes the SD card, then starts reading blocks and streaming them to the Pico. The Pico saves the raw audio data into memory before disabling the FPGA (via a pin separate to the SPI bus) and playing the audio sample in a loop.

= Elements of Complexity
- SD card, driven and read by the FPGA
- I2S between Pico and amplifier
- SPI communication between FPGA and Pico
- Testbenches for Verilog code

= User Guide
TODO: Instructions for flashing audio data onto an SD card
...
On the Raspberry Pi Pico 2, we used CircuitPython to process raw pcm data (place into a buffer) and output it as I2S.

On the FPGA, all the code was written by us.

= List of Protocols Used
- SD card SPI protocol.
  - This is a protocol built on top of SPI. It uses fixed length, 6 byte commands. Most command responses are 1 byte, but some append extra data. We implemented it in Verilog.
- I2S
...
= Design Diagrams
== Wiring
TODO: Maybe sketch something out on paper and scan it
...
TODO: Make a diagram of that state machine

== FPGA Module Hierarchy
#let module-diagram = diagram(
  node-stroke: (thickness: 1pt, paint: blue),
  node-defocus: 0,
  label-size: 8pt,
  spacing: (3.5em, 4em),
  {
    ...
    node((0, 2.75), [sd_card_spi], name: <sd-card-spi>)
    node((1, 2.75), [sd_card_controller], name: <sd-card-controller>)
    node((2, 2.75), [communicator], name: <communicator>)

    node((2, 3.75), [pico_spi_controller], name: <pico-spi>)
    // node()

    edge((0, -2), <top>, "<|-|>", [25Mhz clock\ SD card SPI pins\ Pico SPI pins])
    
    edge(<top>, <pll>, "-|>", [25MHz clock], label-angle: auto, bend: -15deg)
    edge(<top>, <pll>, "<|-", [8MHz clock], label-angle: auto, bend: 15deg)

    edge(<top>, <main>, "<|-|>", [8Mhz clock\ SD card SPI pins\ Pico SPI pins], label-angle: auto)

    edge(<main>, <sd-card-spi>, "-|>", [SPI tx data], label-angle: auto, bend: -30deg)
    edge(<main>, <sd-card-spi>, "<|-", [SPI rx data], label-angle: auto, bend: 0deg)
    
    edge(<main>, <sd-card-controller>, "-|>", [SPI rx data\ Block Address], label-angle: auto, bend: -15deg)
    edge(<main>, <sd-card-controller>, "<|-", [SPI tx data\ Block data], label-angle: auto, bend: 15deg)


    edge(<main>, <communicator>, "<|-", [Pico SPI Pins\ Block address], label-angle: auto, bend: 30deg)
    edge(<main>, <communicator>, "-|>", [Block data], label-angle: auto, bend: 0deg)

    edge(<communicator>, <pico-spi>, "<|-|>", )//[label])
  }
)

#align(center)[#module-diagram]
...
= Acknowledgements
Massive thank you to RB for being a chill TA. Another thanks to Allan Knight for helping with debugging I2S on the fpga.
